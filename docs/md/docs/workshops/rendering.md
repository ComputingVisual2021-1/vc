...
- Como tal, el algoritmo de un pintor inverso no se puede utilizar como una alternativa al z-culling (sin una reingeniería enérgica), excepto como una optimización para el mismo. Por ejemplo, una optimización podría ser mantener los polígonos ordenados según la ubicación x / y y la profundidad z para proporcionar límites, en un esfuerzo por determinar rápidamente si dos polígonos podrían tener una interacción de oclusión.
...
> ```
> z' = \frac{far + near}{far - near} + \frac{1}{z} (\frac{-2 \times far \times near}{far - near})
> ```
...
>:Formula align=center
>```
> z' = 2 \times \frac{z-near}{far-near} - 1
>```


Donde z es el valor viejo de la profundidad en el espacio de la camara y algunas veces llamado w o w'.
...
## Conclusiones
- El z-búfer es un herramienta muy útil en distintos campos de la computación visual, desde el problema de la oclusión entre objetos hasta entrenamiento de IA's o herramientas de visión por computadora.
- Es importante que el búfer tenga una precisión alta, ya que con un tamaño pequeño (menos de 16 bits) se pueden generar artefactos indeseados (No es lo mismo tener un rango de 256 valores con 8 bits a un rango de 4.000'000.000 con 32 bits).
- Es una herramientas muy útil a la hora de ahorrar recursos, evitando el dibujado de píxeles que de no ser detectada su oclusión no serían parte de la escena de todos modos.
- A la hora de su implementación es importante tener cuidado con los loops que se pueden formar entre polígonos que se ocluyen mutuamente.
